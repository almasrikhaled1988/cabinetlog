    ...
    <template v-else-if="overview">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <div class="stat-card dark:stat-card-dark">
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Total Guides</p>
        </div>
        <div class="stat-card dark:stat-card-dark">
          <p class="text-2xl font-bold text-se-green">{{ overview.guides.published }}</p>
        ...
        <div class="stat-card dark:stat-card-dark">
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Workers</p>
        </div>
        <div class="stat-card dark:stat-card-dark">
          <p class="text-2xl font-bold text-amber-600 dark:text-amber-400">{{ overview.comments.open }}</p>
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Open Issues</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="card dark:card-dark p-5">
          <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Most Active Guides</h2>
        ...
        <!-- Problematic Steps -->
        <div class="card dark:card-dark p-5">
          <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Steps with Most Issues</h2>
          ...
          <div v-else class="space-y-3">
            <div v-for="s in problematicSteps" :key="s.step_title" class="flex justify-between items-center text-sm">
              ...
              <span class="text-red-600 dark:text-red-400 text-xs font-medium whitespace-nowrap ml-2">{{ s.issue_count }} issues</span>
            </div>
          </div>
        </div>

        <!-- Worker Activity -->
        <div class="card dark:card-dark p-5 lg:col-span-2">
          <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Worker Activity</h2>
            ...
            <table class="min-w-full text-sm">
              <thead>
                <tr class="text-left text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  ...
                  <th class="pb-3">Steps Completed</th>
                  <th class="pb-3">Guides</th>
                  <th class="pb-3">Last Active</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                <tr v-for="w in workerActivity" :key="w.email">
                  <td class="py-3 text-gray-900 dark:text-white">{{ w.name }}</td>
                  <td class="py-3 text-gray-600 dark:text-gray-300">{{ w.total_steps_completed }}</td>
                  <td class="py-3 text-gray-600 dark:text-gray-300">{{ w.guides_count }}</td>
                  <td class="py-3 text-gray-400 dark:text-gray-500">{{ formatDate(w.last_activity) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
